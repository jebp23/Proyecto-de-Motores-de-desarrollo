...
public class GameManager : MonoBehaviour
{
    ...
    // *** NUEVO PARA BLOQUEAR MÚSICA DURANTE RESPAWN ***
    ...
    IEnumerator RespawnSequence()
    {
        ...
        if (player != null)
        {
            ...
            if (SpawnPoint.I)
                SpawnPoint.I.TeleportImmediate(player);
    ...
    void ClearAroundSpawn(Vector3 center)
    {
        ...
        for (int i = 0; i < enemies.Length; i++)
        {
            ...
            if (d < spawnClearRadius)
                e.WarpAwayFrom(center, spawnClearRadius + 1f);
        }
    }
    ...
    void RecountCollected()
    {
        ...
        for (int i = 0; i < docs.Length; i++)
        {
            ...
            if (d && d.collected && collectedSet.Add(d))
                collectedNotes++;
        }
    }
    ...
    {
        if (progressText)
            progressText.text = $"{collectedNotes}/{totalNotes}";
    }
    ...
    private void Update()
    {
        ...
        // *** NUEVO: no permitir chase durante respawn ***
        ...
        if (sanitySys)
            sanityPercent = sanitySys.CurrentSanity / sanitySys.MaxSanity;

        var monsters = FindObjectsByType<EnemyMonster>(FindObjectsSortMode.None);

        foreach (var m in monsters)
        {
            if (m.CurrentlyDetecting)
            {
                anyDetecting = true;
                break;
            }
